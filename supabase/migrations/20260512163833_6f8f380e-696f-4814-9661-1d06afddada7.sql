-- Fix false positive in payment_reconciliation: task_active_without_escrow
-- - include 'completion_requested' as active task state
-- - rely on per-task active escrow count (held/released) instead of per-row escrow_id,
--   so cancelled/failed orders for a task that already has another valid escrow
--   no longer trigger task_active_without_escrow
...
with
  ...
  task_active_escrow_counts as (
    select task_id, count(*)::int as active_escrow_count
    from public.escrow_transactions
    where task_id is not null
  ...
  base as (
    select
      ...
      coalesce(taec.active_escrow_count, 0)       as active_escrow_count_for_task,
    ...
    left join task_active_escrow_counts taec
      on taec.task_id = coalesce(o.task_id, e.task_id, p.task_id)
...
select
  ...
  case
    ...
    -- task_active_without_escrow:
    --   only when task is in an active state AND no held/released escrow exists for the task at all.
    --   Cancelled/failed orders no longer trigger this because we look at the task-level
    --   active escrow count, not the per-row escrow_id (which is null for cancelled-order rows).
    when task_status in ('in_progress', 'completed', 'completion_requested')
         and active_escrow_count_for_task = 0
...
comment on view public.payment_reconciliation is
  'Admin-only reconciliation view across orders, escrow_transactions, payouts, tasks, proposals. mismatch_reason flags financial inconsistencies. task_active_without_escrow uses per-task active escrow count and ignores cancelled/failed orders.';
