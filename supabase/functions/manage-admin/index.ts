...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isSuperAdmin) {
      return new Response(JSON.stringify({ error: 'Only super admin can manage roles' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (!allowedRoles.includes(role)) {
      return new Response(JSON.stringify({ error: 'Invalid role. Cannot assign super_admin via this endpoint.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    // Prevent self-modification
    const callerEmail = user.email?.toLowerCase()
    if (callerEmail === email.trim().toLowerCase()) {
      return new Response(JSON.stringify({ error: 'Cannot modify your own roles' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    const { data: targetIsSuperAdmin } = await adminClient.rpc('has_role', { _user_id: profile.user_id, _role: 'super_admin' })
    if (targetIsSuperAdmin) {
      return new Response(JSON.stringify({ error: 'Cannot modify super admin roles' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (action === 'remove') {
      ...
      if (existingRole) {
        return new Response(JSON.stringify({ error: `User already has role: ${role}`, display_name: profile.display_name }), { status: 409, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
      }
